...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public void notifyDataSetChanged(int position) {
		if (tabs == null || tabs.size() <= position)
			return;
		String str = (String) pager.getAdapter().getPageTitle(position);
		tabs.get(position).setText(str);
	}

	private void notifyTextSetChanged(int position) {
		if (tabs == null || tabs.size() <= position)
			return;
		for (TextView tab : tabs) {
			tab.setTextColor(tabTextColor);
		}
		tabs.get(position).setTextColor(selectedTabTextColor);
	}
